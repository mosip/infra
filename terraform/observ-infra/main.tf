...
# AWS Observation Infrastructure (uses same modules as infra with minimal resources)
module "aws_observ_infra" {
  count  = var.cloud_provider == "aws" ? 1 : 0
  source = "./aws"
  
  # Pass all variables to the AWS module (same as infra but with different values)
...
# GCP Observation Infrastructure
module "gcp_observ_infra" {
