...
if [ "${HELMSMAN_MODE:-}" = "dry-run" ]; then
  echo "[DRY-RUN] Skipping alerting-setup.sh hook (no real resources exist in dry-run)"
  exit 0
fi
...
function installing_alerting() {
  ...
  kubectl patch secret alertmanager-rancher-monitoring-alertmanager -n $NS  --patch="{\"data\": { \"alertmanager.yaml\": \"$(cat $WORKDIR/utils/alerting/alertmanager.yaml |base64 |tr -d '\n' )\" }}"
  ...
  kubectl patch Prometheus rancher-monitoring-prometheus -n $NS --patch-file $WORKDIR/utils/alerting/patch-cluster-name.yaml --type=merge
  echo Applying custom alerts
  kubectl apply -f $WORKDIR/utils/alerting/custom-alerts/
  return 0
}
