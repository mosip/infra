...
if [ "${HELMSMAN_MODE:-}" = "dry-run" ]; then
  echo "[DRY-RUN] Skipping softhsm-mock-identity-system-postinstall.sh hook (no real resources exist in dry-run)"
  exit 0
fi
...
function postinstall_softhsm_mock_identity() {
  ...
  echo "Config-server restart initiated, continuing deployment..."
