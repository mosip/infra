...
# Override postgres-config with MOSIPID-specific DB values
...
# Remove stale pre-consolidation env var names if still present, so they don't
# linger pointing at secrets that no longer exist (esignet-mosipid1 was renamed
# to esignet-mosipid; esignet-mosipid2 was removed).
for OLD_NAME in MOSIP_CAPTCHA_GOOGLERECAPTCHAV2_SECRET_ESIGNETMOSIPID1 MOSIP_CAPTCHA_GOOGLERECAPTCHAV2_SECRET_ESIGNETMOSIPID2; do
