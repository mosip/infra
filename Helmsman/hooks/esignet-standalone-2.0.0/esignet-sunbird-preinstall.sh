...
# Override postgres-config with Sunbird-specific DB values (2.0.0: isolated database)
...
# pointing at a secret that no longer exists (esignet-captcha-go-sunbird was renamed
# to esignet-captcha-sunbird).
OLD_ENV_INDEX=$(kubectl -n "$CAPTCHA_NS" get deployment captcha \
  -o jsonpath="{range .spec.template.spec.containers[0].env[*]}{.name}{'\n'}{end}" 2>/dev/null \
  | grep -n -x "MOSIP_CAPTCHA_GOOGLERECAPTCHAV2_SECRET_ESIGNETGOSUNBIRD" | cut -d: -f1 || echo "")
if [[ -n "$OLD_ENV_INDEX" ]]; then
  OLD_ENV_ARRAY_INDEX=$((OLD_ENV_INDEX - 1))
  echo "Removing stale MOSIP_CAPTCHA_GOOGLERECAPTCHAV2_SECRET_ESIGNETGOSUNBIRD env var (index $OLD_ENV_ARRAY_INDEX)"
