...
- **New database entries MUST include `su` block**: `0.0.1-develop` chart `job.yaml` accesses `$dbVal.su.secret.name` for every enabled entry. Entries that exist in the chart's default values (e.g. `mosip_esignet`, `mosip_mockidentitysystem`) inherit `su` automatically; entries that are NEW (e.g. `mosip_esignet_cre`, `mosip_esignet_qa11`, `mosip_esignet_sunbird`) have no defaults and will fail with `nil pointer evaluating interface {}.secret` unless you explicitly add `databases.<key>.su.user: "postgres"`, `databases.<key>.su.secret.name: "postgres-postgresql"`, `databases.<key>.su.secret.key: "postgres-password"` in the DSF `set:` block
